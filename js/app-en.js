  ...
  try {
    ...
    if (data.results.length > 0) {
      ...
      data.results.forEach((movie) => {
        images.push(`url('https://image.tmdb.org/t/p/w1920_and_h1080_bestv2/${movie.backdrop_path}'`);
        ...
        movieCategory.textContent = movie.genre_ids
          .map((id) => genres[id])
          .join(", ");
        ...
        movieRating.addEventListener("click", () => openpopup(movie.id));
        movieTitle.addEventListener("click", () => openpopup(movie.id));
...
async function openpopup(movieId) {
  ...
  try {
    ...
    newwinow.innerHTML = `<div class="popup__card">    
        <iframe class="popup__video" src="https://www.youtube-nocookie.com/embed/${videoId}?controls=0&autoplay=1" title="YouTube video player" allow="autoplay" frameborder="0"></iframe>
        ...
        <ul class="popup__movie--info">
            ...
            <li class="popup__movie--runtime"><span class="accent">Duration: </span>${
              detResp.runtime
            } minutes</li>
            <li class="popup__movie--link"><span class="accent">Rating: </span> ${
              detResp.vote_average
            } (${detResp.vote_count} votes)</li>
            <li class="popup__movie--overview"><span class="accent">Overview: </span>${
              detResp.overview
            }</li>
        </ul>
...
function handleNextPrevButtonClick(event) {
  if (event.target.classList.contains("number__next")) {
    ...
    const elements = document.querySelectorAll(
      ".number__elements .number__button"
    );
    const activeElement = document.querySelector(
      ".number__elements .number__button.number__active"
    );

    if (activeElement) {
      const nextElement = Array.from(elements).find(
        (element) =>
          parseInt(element.innerText) > parseInt(activeElement.innerText)
      );
